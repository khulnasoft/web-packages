...
<script>
import { addClass, find, findSingle, getAttribute, removeClass, setAttribute } from '@khulnasoft/utils/dom';
import { localeComparator, sort } from '@khulnasoft/utils/object';
    ...
    methods: {
        ...
        createStyle() {
            if (!this.carouselStyle) {
                ...
                setAttribute(this.carouselStyle, 'nonce', this.$khulnasoft?.config?.csp?.nonce);
        ...
        ariaSlideNumber(value) {
            return this.$khulnasoft.config.locale.aria ? this.$khulnasoft.config.locale.aria.slideNumber.replace(/{slideNumber}/g, value) : undefined;
        },
        ariaPageLabel(value) {
            return this.$khulnasoft.config.locale.aria ? this.$khulnasoft.config.locale.aria.pageLabel.replace(/{page}/g, value) : undefined;
        }
    },
    computed: {
        totalIndicators() {
        ...
        ariaSlideLabel() {
            return this.$khulnasoft.config.locale.aria ? this.$khulnasoft.config.locale.aria.slide : undefined;
        },
        ariaPrevButtonLabel() {
            return this.$khulnasoft.config.locale.aria ? this.$khulnasoft.config.locale.aria.prevPageLabel : undefined;
        },
        ariaNextButtonLabel() {
            return this.$khulnasoft.config.locale.aria ? this.$khulnasoft.config.locale.aria.nextPageLabel : undefined;
        },
        empty() {
            return !this.value || this.value.length === 0;
        },
        emptyMessageText() {
            return this.$khulnasoft.config?.locale?.emptyMessage || '';
        }
    },
    components: {
        Button,
