<script>
import { Theme, ThemeService } from '@khulnasoft/styled';
import { findSingle, isClient } from '@khulnasoft/utils/dom';
import { getKeyValue, isArray, isFunction, isNotEmpty, isString, resolve, toFlatCase } from '@khulnasoft/utils/object';
import { uuid } from '@khulnasoft/utils/uuid';
import Base from '@khulnasoft/core/base';
...
export default {
    ...
    beforeCreate() {
        ...
        const _useptInConfig = this.$khulnasoftConfig?.pt?.['_usept'];
        const originalValueInConfig = _useptInConfig ? this.$khulnasoft?.config?.pt?.originalValue : undefined;
        const valueInConfig = _useptInConfig ? this.$khulnasoft?.config?.pt?.value : this.$khulnasoft?.config?.pt;
    ...
    methods: {
        ...
        _getPTValue(obj = {}, key = '', params = {}, searchInDefaultPT = true) {
            ...
            const { mergeSections = true, mergeProps: useMergeProps = false } = this._getPropValue('ptOptions') || this.$khulnasoftConfig?.ptOptions || {};
        ...
        _usePT(pt, callback, key, params) {
            ...
            if (pt?.hasOwnProperty('_usept')) {
                const { mergeSections = true, mergeProps: useMergeProps = false } = pt['_usept'] || this.$khulnasoftConfig?.ptOptions || {};
    ...
    computed: {
        globalPT() {
            return this._getPT(this.$khulnasoftConfig?.pt, undefined, (value) => resolve(value, { instance: this }));
        },
        defaultPT() {
            return this._getPT(this.$khulnasoftConfig?.pt, undefined, (value) => this._getOptionValue(value, this.$name, { ...this.$params }) || resolve(value, { ...this.$params }));
        },
        isUnstyled() {
            return this.unstyled !== undefined ? this.unstyled : this.$khulnasoftConfig?.unstyled;
        },
        $inProps() {
        ...
        $theme() {
            return this.$khulnasoftConfig?.theme;
        },
        $style() {
        ...
        $styleOptions() {
            return { nonce: this.$khulnasoftConfig?.csp?.nonce };
        },
        $khulnasoftConfig() {
            return this.$khulnasoft?.config;
        },
        $name() {
