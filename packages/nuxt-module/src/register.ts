...
import { isNotEmpty, isString, resolve } from '@khulnasoft/utils/object';
import type { MetaType } from '@khulnasoft/metadata';
import { components, composables, directives } from '@khulnasoft/metadata';
import type { PrimeVueConfiguration } from 'khulnasoft/config';
    ...
    return [
        {
            name: 'PrimeVue',
            as: 'PrimeVue',
            from: resolvePath({ name: 'PrimeVue', as: 'PrimeVue', from: `khulnasoft/config`, type: 'config' })
        }
    ];
}
...
function registerServices(resolvePath: any, registered: any) {
    ...
    return [...services].map((service) => ({
        name: service,
        as: service,
        from: resolvePath({ name: service, as: service, from: `khulnasoft/${service.toLowerCase()}`, type: 'service' })
    }));
}
    ...
    const styles: MetaType[] = [
        {
            name: 'BaseStyle',
            as: 'BaseStyle',
            from: resolvePath({ name: 'BaseStyle', as: 'BaseStyle', from: '@khulnasoft/core/base/style', type: 'style' })
        }
    ];
    ...
    if (!moduleOptions.autoImport && !options?.unstyled) {
        if (isNotEmpty(registered?.components)) {
            styles.push({
                ...
                from: resolvePath({ name: 'BaseComponentStyle', as: 'BaseComponentStyle', from: '@khulnasoft/core/basecomponent/style', type: 'style' })
            });
        }
        ...
        [registered.components, registered.directives]
            ...
            .forEach((item: any) =>
                styles.push({
                    name: `${item.as}Style`,
                    as: `${item.as}Style`,
                    from: resolvePath({ name: `${item.as}Style`, as: `${item.as}Style`, from: `khulnasoft/${item.as.toLowerCase()}/style`, type: 'style' })
                })
            );
    }
