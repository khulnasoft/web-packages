            ...
            if (name === folderName || name === `${folderName}icon`) {
                ...
                if (validModuleFolder) {
                    modules.esm.push(
                        `
// ${fileName}
export * from '@khulnasoft/icons/${folderName}';
export { default as ${fileName} } from '@khulnasoft/icons/${folderName}';
`
                    );
                }
                ...
                try {
                    fs.readdirSync(folderPath + '/style').forEach((file) => {
                        ...
                        if (subName === `${folderName}style`) {
                            ...
                            if (validModuleFolder) {
                                modules.esm.push(
                                    `export * from '@khulnasoft/icons/${folderName}/style';
export { default as ${subFileName} } from '@khulnasoft/icons/${folderName}/style';
`
                                );
                            }
                        }
