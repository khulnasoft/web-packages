    ...
    <div :ref="containerRef" :id="id" :class="cx('root')" v-bind="ptmi('root')">
        ...
        <slot :id="id" :name="$slots.button ? 'button' : 'menubutton'" :class="cx('button')" :toggleCallback="(event) => menuButtonClick(event)">
            <a
                ...
                :aria-controls="id"
                :aria-label="$khulnasoft.config.locale.aria?.navigation"
...
<script>
import { findSingle, focus, isTouchDevice } from '@khulnasoft/utils/dom';
    ...
    methods: {
        ...
        toggle(event) {
            if (this.mobileActive) {
                ...
                ZIndex.set('menu', this.menubar, this.$khulnasoft.config.zIndex.menu);
                setTimeout(() => {
                    this.show();
                }, 1);
            }
