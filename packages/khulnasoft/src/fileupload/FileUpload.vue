...
<script>
import { addClass, removeClass } from '@khulnasoft/utils/dom';
import PlusIcon from '@khulnasoft/icons/plus';
import TimesIcon from '@khulnasoft/icons/times';
import UploadIcon from '@khulnasoft/icons/upload';
import Button from 'khulnasoft/button';
import Message from 'khulnasoft/message';
import ProgressBar from 'khulnasoft/progressbar';
...
export default {
    ...
    methods: {
        ...
        formatSize(bytes) {
            const k = 1024;
            const dm = 3;
            const sizes = this.$khulnasoft.config.locale?.fileSizeTypes || ['B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    ...
    computed: {
        ...
        basicFileChosenLabel() {
            if (this.auto) return this.chooseButtonLabel;
                ...
                return this.$khulnasoft.config.locale?.fileChosenMessage?.replace('{0}', this.files.length);
            }

            return this.$khulnasoft.config.locale?.noFileChosenMessage || '';
        },
        hasFiles() {
        ...
        chooseButtonLabel() {
            return this.chooseLabel || this.$khulnasoft.config.locale.choose;
        },
        uploadButtonLabel() {
            return this.uploadLabel || this.$khulnasoft.config.locale.upload;
        },
        cancelButtonLabel() {
            return this.cancelLabel || this.$khulnasoft.config.locale.cancel;
        },
        completedLabel() {
            return this.$khulnasoft.config.locale.completed;
        },
        pendingLabel() {
            return this.$khulnasoft.config.locale.pending;
        }
    },
    components: {
        Button,
