...
<script>
import { FilterOperator } from '@khulnasoft/core/api';
import BaseComponent from '@khulnasoft/core/basecomponent';
import { ConnectedOverlayScrollHandler, UniqueComponentId } from '@khulnasoft/core/utils';
import { getAttribute, focus, addStyle, absolutePosition, isTouchDevice } from '@khulnasoft/utils/dom';
import { ZIndex } from '@khulnasoft/utils/zindex';
import FilterIcon from '@khulnasoft/icons/filter';
import FilterSlashIcon from '@khulnasoft/icons/filterslash';
import PlusIcon from '@khulnasoft/icons/plus';
import TrashIcon from '@khulnasoft/icons/trash';
import Button from 'khulnasoft/button';
import FocusTrap from 'khulnasoft/focustrap';
import OverlayEventBus from 'khulnasoft/overlayeventbus';
import Portal from 'khulnasoft/portal';
import Select from 'khulnasoft/select';
    ...
    computed: {
        ...
        matchModes() {
            return (
                ...
                this.$khulnasoft.config.filterMatchModeOptions[this.type].map((key) => {
                    return { label: this.$khulnasoft.config.locale[key], value: key };
                })
            );
        },
        isShowMatchModes() {
        ...
        operatorOptions() {
            return [
                { label: this.$khulnasoft.config.locale.matchAll, value: FilterOperator.AND },
                { label: this.$khulnasoft.config.locale.matchAny, value: FilterOperator.OR }
            ];
        },
        noFilterLabel() {
            return this.$khulnasoft.config.locale ? this.$khulnasoft.config.locale.noFilter : undefined;
        },
        isShowOperator() {
        ...
        removeRuleButtonLabel() {
            return this.$khulnasoft.config.locale ? this.$khulnasoft.config.locale.removeRule : undefined;
        },
        addRuleButtonLabel() {
            return this.$khulnasoft.config.locale ? this.$khulnasoft.config.locale.addRule : undefined;
        },
        isShowAddConstraint() {
            return this.showAddButton && this.filters[this.field].operator && this.fieldConstraints && this.fieldConstraints.length < this.maxConstraints;
        },
        clearButtonLabel() {
            return this.$khulnasoft.config.locale ? this.$khulnasoft.config.locale.clear : undefined;
        },
        applyButtonLabel() {
            return this.$khulnasoft.config.locale ? this.$khulnasoft.config.locale.apply : undefined;
        },
        columnFilterButtonAriaLabel() {
            return this.$khulnasoft.config.locale ? (this.overlayVisible ? this.$khulnasoft.config.locale.showFilterMenu : this.$khulnasoft.config.locale.hideFilterMenu) : undefined;
        },
        filterOperatorAriaLabel() {
            return this.$khulnasoft.config.locale ? this.$khulnasoft.config.locale.filterOperator : undefined;
        },
        filterRuleAriaLabel() {
            return this.$khulnasoft.config.locale ? this.$khulnasoft.config.locale.filterConstraint : undefined;
        },
