...
import { isNotEmpty } from '@khulnasoft/utils';
...
export default defineNuxtModule<ModuleOptions>({
    ...
    setup(moduleOptions, nuxt) {
        ...
        nuxt.options.runtimeConfig.public.khulnasoft = {
            ...moduleOptions,
            ...registered
        };

        //nuxt.options.build.transpile.push('nuxt');
        nuxt.options.build.transpile.push('khulnasoft');
...
${
    hasTheme
        ? `import { Theme } from '@khulnasoft/styled';
${importTheme ? `import ${importTheme.as} from '${normalize(importTheme.from)}';\n` : ''}`
        : ''
}

const runtimeConfig = useRuntimeConfig();
const config = runtimeConfig?.public?.khulnasoft ?? {};
...
`;
        ...
        nuxt.options.alias['#khulnasoft-style'] = addTemplate({
            filename: 'khulnasoft-style.mjs',
        ...
        addPluginTemplate({
            filename: 'khulnasoft-plugin.mjs',
            getContents() {
                return `
...
export default defineNuxtPlugin(({ vueApp }) => {
  ...
  ${!autoImport && registered.directives.map((directive: MetaType) => `vueApp.directive('${directive.name}', ${directive.as});`).join('\n')}
        ...
        nuxt.hook('nitro:config', async (config) => {
            ...
            config.virtual['#khulnasoft-style'] = styleContent;
