...
import { getKeyValue, isArray, isEmpty, isFunction, isObject, isString, resolve, toCapitalCase, toFlatCase } from '@khulnasoft/utils/object';
...
import PrimeVueService from '@khulnasoft/core/service';
import { mergeProps } from 'vue';

const BaseDirective = {
    _getMeta: (...args) => [isObject(args[0]) ? undefined : args[0], resolve(isObject(args[0]) ? args[0] : args[1])],
    _getConfig: (binding, vnode) => (binding?.instance?.$khulnasoft || vnode?.ctx?.appContext?.config?.globalProperties?.$khulnasoft)?.config,
    ...
    _getPTValue: (instance = {}, obj = {}, key = '', params = {}, searchInDefaultPT = true) => {
        ...
        const { mergeSections = true, mergeProps: useMergeProps = false } = instance.binding?.value?.ptOptions || instance.$khulnasoftConfig?.ptOptions || {};
    ...
    _usePT: (instance = {}, pt, callback, key, params) => {
        ...
        if (pt?.hasOwnProperty('_usept')) {
            const { mergeSections = true, mergeProps: useMergeProps = false } = pt['_usept'] || instance.$khulnasoftConfig?.ptOptions || {};
    ...
    _extend: (name, options = {}) => {
        const handleHook = (hook, el, binding, vnode, prevVnode) => {
            ...
            el._$instances[name] = {
                ...
                theme: () => el.$instance?.$khulnasoftConfig?.theme,
        ...
        const handleWatch = (el) => {
            ...
            // for 'config'
            watchers?.['config']?.call(el.$instance, el.$instance?.$khulnasoftConfig);
            PrimeVueService.on('config:change', ({ newValue, oldValue }) => watchers?.['config']?.call(el.$instance, newValue, oldValue));

            // for 'config.ripple'
            watchers?.['config.ripple']?.call(el.$instance, el.$instance?.$khulnasoftConfig?.ripple);
