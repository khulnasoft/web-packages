    ...
    <span ref="container" :id="d_id" :class="cx('root')" :style="sx('root')" v-bind="ptmi('root')">
        ...
        <slot v-if="showIcon && iconDisplay === 'button' && !inline" name="dropdownbutton" :toggleCallback="onButtonClick">
            <button
                ...
                @click="onButtonClick"
                type="button"
                :aria-label="$khulnasoft.config.locale.chooseDate"
                aria-haspopup="dialog"
                :aria-expanded="overlayVisible"
                :aria-controls="panelId"
                v-bind="ptm('dropdown')"
                            ...
                            <div v-for="(month, groupIndex) of months" :key="month.month + month.year" :class="cx('calendar')" v-bind="ptm('calendar')">
                                <div :class="cx('header')" v-bind="ptm('header')">
                                    ...
                                    <Button
                                        ...
                                        :disabled="disabled"
                                        :aria-label="currentView === 'year' ? $khulnasoft.config.locale.prevDecade : currentView === 'month' ? $khulnasoft.config.locale.prevYear : $khulnasoft.config.locale.prevMonth"
                                        :unstyled="unstyled"
                                    ...
                                    <div :class="cx('title')" v-bind="ptm('title')">
                                        <template v-if="$khulnasoft.config.locale.showMonthAfterYear">
                                            <button
                                                ...
                                                :aria-label="$khulnasoft.config.locale.chooseYear"
                                            ...
                                            <button
                                                ...
                                                :disabled="switchViewButtonDisabled"
                                                :aria-label="$khulnasoft.config.locale.chooseMonth"
                                    ...
                                    <Button
                                        ...
                                        :disabled="disabled"
                                        :aria-label="currentView === 'year' ? $khulnasoft.config.locale.nextDecade : currentView === 'month' ? $khulnasoft.config.locale.nextYear : $khulnasoft.config.locale.nextMonth"
                                        :unstyled="unstyled"
                    ...
                    <div v-if="(showTime || timeOnly) && currentView === 'date'" :class="cx('timePicker')" v-bind="ptm('timePicker')">
                        <div :class="cx('hourPicker')" v-bind="ptm('hourPicker')" data-pc-group-section="timepickerContainer">
                            <Button
                                ...
                                :aria-label="$khulnasoft.config.locale.nextHour"
                                :unstyled="unstyled"
                            ...
                            <Button
                                ...
                                :aria-label="$khulnasoft.config.locale.prevHour"
                                :unstyled="unstyled"
                                @mousedown="onTimePickerElementMouseDown($event, 0, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                                @keydown="onContainerButtonKeydown"
                        ...
                        <div :class="cx('minutePicker')" v-bind="ptm('minutePicker')" data-pc-group-section="timepickerContainer">
                            <Button
                                :class="cx('pcIncrementButton')"
                                :aria-label="$khulnasoft.config.locale.nextMinute"
                                :disabled="disabled"
                                :unstyled="unstyled"
                            ...
                            <Button
                                :class="cx('pcDecrementButton')"
                                :aria-label="$khulnasoft.config.locale.prevMinute"
                                :disabled="disabled"
                                @mousedown="onTimePickerElementMouseDown($event, 1, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                                @keydown="onContainerButtonKeydown"
                        ...
                        <div v-if="showSeconds" :class="cx('secondPicker')" v-bind="ptm('secondPicker')" data-pc-group-section="timepickerContainer">
                            <Button
                                :class="cx('pcIncrementButton')"
                                :aria-label="$khulnasoft.config.locale.nextSecond"
                                :disabled="disabled"
                                :unstyled="unstyled"
                            ...
                            <Button
                                :class="cx('pcDecrementButton')"
                                :aria-label="$khulnasoft.config.locale.prevSecond"
                                :disabled="disabled"
                                :unstyled="unstyled"
                                @mousedown="onTimePickerElementMouseDown($event, 2, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                        ...
                        <div v-if="hourFormat == '12'" :class="cx('ampmPicker')" v-bind="ptm('ampmPicker')">
                            <Button
                                :class="cx('pcIncrementButton')"
                                :aria-label="$khulnasoft.config.locale.am"
                                :disabled="disabled"
                                :unstyled="unstyled"
                            ...
                            <span v-bind="ptm('ampm')" data-pc-group-section="timepickerlabel">{{ pm ? $khulnasoft.config.locale.pm : $khulnasoft.config.locale.am }}</span>
                            <Button
                                :class="cx('pcDecrementButton')"
                                :aria-label="$khulnasoft.config.locale.pm"
                                :disabled="disabled"
                                @click="toggleAMPM($event)"
                                @keydown="onContainerButtonKeydown"
                                v-bind="timepickerButtonProps"
...
<script>
import { absolutePosition, addStyle, find, findSingle, getAttribute, getFocusableElements, getIndex, getOuterWidth, isTouchDevice, relativePosition, setAttribute } from '@khulnasoft/utils/dom';
import { localeComparator } from '@khulnasoft/utils/object';
import { ZIndex } from '@khulnasoft/utils/zindex';
import { ConnectedOverlayScrollHandler, UniqueComponentId } from '@khulnasoft/core/utils';
import CalendarIcon from '@khulnasoft/icons/calendar';
import ChevronDownIcon from '@khulnasoft/icons/chevrondown';
import ChevronLeftIcon from '@khulnasoft/icons/chevronleft';
import ChevronRightIcon from '@khulnasoft/icons/chevronright';
import ChevronUpIcon from '@khulnasoft/icons/chevronup';
import Button from 'khulnasoft/button';
import InputText from 'khulnasoft/inputtext';
import OverlayEventBus from 'khulnasoft/overlayeventbus';
import Portal from 'khulnasoft/portal';
import Ripple from 'khulnasoft/ripple';
...
export default {
    ...
    methods: {
        ...
        onOverlayEnter(el) {
            ...
            if (this.autoZIndex) {
                ZIndex.set('overlay', el, this.baseZIndex || this.$khulnasoft.config.zIndex.overlay);
            }
                    ...
                    if (literal) {
                        ...
                        switch (format.charAt(iFormat)) {
                            case 'd':
                                ...
                                break;
                            case 'D':
                                output += formatName('D', date.getDay(), this.$khulnasoft.config.locale.dayNamesShort, this.$khulnasoft.config.locale.dayNames);
                                break;
                            case 'o':
                            ...
                            case 'm':
                                ...
                                break;
                            case 'M':
                                output += formatName('M', date.getMonth(), this.$khulnasoft.config.locale.monthNamesShort, this.$khulnasoft.config.locale.monthNames);
                                break;
                            case 'y':
        ...
        formatTime(date) {
            ...
            if (this.hourFormat === '12') {
                output += date.getHours() > 11 ? ` ${this.$khulnasoft.config.locale.pm}` : ` ${this.$khulnasoft.config.locale.am}`;
            }
        ...
        populateTime(value, timeString, ampm) {
            ...
            this.pm = ampm === this.$khulnasoft.config.locale.pm || ampm === this.$khulnasoft.config.locale.pm.toLowerCase();
                ...
                if (literal) {
                    ...
                    switch (format.charAt(iFormat)) {
                        case 'd':
                            ...
                            break;
                        case 'D':
                            getName('D', this.$khulnasoft.config.locale.dayNamesShort, this.$khulnasoft.config.locale.dayNames);
                            break;
                        case 'o':
                            doy = getNumber('o');
                            break;
                        case 'm':
                            ...
                            break;
                        case 'M':
                            month = getName('M', this.$khulnasoft.config.locale.monthNamesShort, this.$khulnasoft.config.locale.monthNames);
                            break;
                        case 'y':
                            year = getNumber('y');
                            break;
        ...
        getMonthName(index) {
            return this.$khulnasoft.config.locale.monthNames[index];
        },
        getYear(month) {
        ...
        createResponsiveStyle() {
            if (this.numberOfMonths > 1 && this.responsiveOptions && !this.isUnstyled) {
                if (!this.responsiveStyleElement) {
                    ...
                    setAttribute(this.responsiveStyleElement, 'nonce', this.$khulnasoft?.config?.csp?.nonce);
    ...
    computed: {
        ...
        weekDays() {
            let weekDays = [];
            let dayIndex = this.$khulnasoft.config.locale.firstDayOfWeek;

            for (let i = 0; i < 7; i++) {
                weekDays.push(this.$khulnasoft.config.locale.dayNamesMin[dayIndex]);
        ...
        sundayIndex() {
            return this.$khulnasoft.config.locale.firstDayOfWeek > 0 ? 7 - this.$khulnasoft.config.locale.firstDayOfWeek : 0;
        },
        datePattern() {
            return this.dateFormat || this.$khulnasoft.config.locale.dateFormat;
        },
        monthPickerValues() {
            ...
            for (let i = 0; i <= 11; i++) {
                monthPickerValues.push({ value: this.$khulnasoft.config.locale.monthNamesShort[i], selectable: isSelectableMonth(i) });
            }
        ...
        todayLabel() {
            return this.$khulnasoft.config.locale.today;
        },
        clearLabel() {
            return this.$khulnasoft.config.locale.clear;
        },
        weekHeaderLabel() {
            return this.$khulnasoft.config.locale.weekHeader;
        },
        monthNames() {
            return this.$khulnasoft.config.locale.monthNames;
        },
